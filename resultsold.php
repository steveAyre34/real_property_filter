...
<?php
	...
	*****************************************************************************************/
	...
	TRIM(CONCAT($owner.prefix_dir, ' ', $owner.mail_st_nbr, ' ', $owner.mail_st_rte, ' ',  $owner.owner_mail_st_suff, ' ', $owner.post_dir)) AS AddressLine1, 
	CASE 
		WHEN $owner.po_box != '' THEN TRIM(CONCAT('PO BOX ', $owner.po_box, $owner.owner_unit_name, ' ', $owner.owner_unit_nbr, $owner.owner_addl_addr))
		ELSE TRIM(CONCAT($owner.owner_unit_name, ' ', $owner.owner_unit_nbr, $owner.owner_addl_addr)) 
	END AS AddressLine2,
...
QRY;
	...
	if(isset($output_street_addr) && $output_street_addr = TRUE)
		$sqlSEL .= <<<QRY
...
QRY;
	...
	if(isset($output_street_addr) && $output_street_addr = TRUE)
	{
	    $street_addr = array( 'StreetAddressLine1', 'StreetAddressLine2', 'StreetAddressCity', 'StreetAddressSate', 'StreetAddressZip');
	...
	**/
	...
	$rpsql = $sqlSEL . $sqlFROM . $sqlWHERE . $sqlGROUP;
		...
		$sqlGROUP = "GROUP BY {$owner}.mail_zip, {$owner}.mail_city, {$owner}.mail_st_rte, {$owner}.mail_st_nbr ORDER BY ID"; 
		$rpsql = $sqlSEL . $sqlFROM . $sqlWHERE . $sqlGROUP;
        print('Executing query: please wait. This may take a while depending on the complexity of the query...');
        ob_flush();
        flush();
