...
<?php
	...
	**/
	...
	if(isset($_POST['improvements_all'])){
			$improvements_all = $_POST['improvements_all'];
			debug("All pools groups:");
			if(DEBUG) dump($improvements_all);
	}
	...
	if(isset($_POST['bools'])){
		...
		// $bool_array = $_POST['bools'];
		// $output_cols = array_merge($output_cols, $bool_array);
	...
	if(isset($_POST['bounds'])){
		...
		$bounds_array = $_POST['bounds'];
		$output_cols = array_merge($output_cols, $bounds_array);
		if(DEBUG) dump($criteria);
		foreach($criteria as $field){
	...
	$land = $county . "_land";
	$improvement = $county . "_improvement";
...
QRY;
	...
	if(isset($output_cols)){
		array_push($output_cols, "CRRT","dp3");
	...
	if(isset($prop_groups)){
		...
		if(in_array('all_res', $prop_groups) && (in_array('all_com', $prop_groups))){
			...
			if(in_array('all_agri', $prop_groups)){
				$prop_clauses []= " prop_class LIKE '1%' " ;
			}
			if(in_array('all_vacant', $prop_groups)){
				$prop_clauses []= " prop_class LIKE '3%' " ;
			}
			if(in_array('all_recreational', $prop_groups)){
				$prop_clauses []= " prop_class LIKE '5%' " ;
			}
			if(in_array('all_600', $prop_groups)){
				$prop_clauses []= " prop_class LIKE '6%' " ;
			}
			if(in_array('all_manu', $prop_groups)){
				$prop_clauses []= " prop_class LIKE '7%' " ;
			}
			if(in_array('all_800', $prop_groups)){
				$prop_clauses []= " prop_class LIKE '8%' " ;
			}
			if(in_array('all_forest', $prop_groups)){
				$prop_clauses []= " prop_class LIKE '9%' " ;
			}
			
			$sqlWHERE .= implode(" AND ", $prop_clauses) . ") ";
		}
	}
	// test code for all pools
	// if(isset($improvements_all)){
		// $sqlSEL = "SELECT improve_nbr";
		// $sqlFROM = " FROM {$improvement} ";
	// }
	if(isset($improvements_all)){
		$pool_clauses = array();
		#$sqlSEL .= "," . {$county}_improvement.improve_nbr. " ";
		$sqlFROM .= "LEFT JOIN {$county}_improvement ON ({$site}.muni_code = {$county}_improvement.muni_code AND {$site}.parcel_id = {$county}_improvement.parcel_id) ";
		
		$sqlWHERE .= " AND (";
		if(in_array('all_pools', $improvements_all)){
			$sqlWHERE .= " structure_cd LIKE 'LS%') " ;
		} else {
			if(in_array('all_farm_barn', $improvements_all)){
                                $pool_clauses []= " structure_cd LIKE 'FB%' OR structure_cd LIKE 'FP%' " ;
                        }
			if(in_array('all_mobile', $improvements_all)){
					$pool_clauses []= " structure_cd LIKE 'MH%' " ;
			}
			if(in_array('all_sheds', $improvements_all)){
					$pool_clauses []= " structure_cd LIKE 'FC%' " ;
			}
			if(in_array('all_patio', $improvements_all)){
					$pool_clauses []= " structure_cd LIKE 'LP%' " ;
			}
			if(in_array('all_tennis', $improvements_all)){
					$pool_clauses []= " structure_cd LIKE 'TC%' " ;
			}
			if(in_array('all_garage', $improvements_all)){
					$pool_clauses []= " structure_cd LIKE 'RG%' " ;
			}
			if(in_array('all_canopy', $improvements_all)){
					$pool_clauses []= " structure_cd LIKE 'CP%' " ;
			}
			if(in_array('all_porches', $improvements_all)){
					$pool_clauses []= " structure_cd LIKE 'RP%' " ;
			}
			if(in_array('all_golf', $improvements_all)){
					$pool_clauses []= " structure_cd LIKE 'GC%' " ;
			}
			if(in_array('all_cold_storage', $improvements_all)){
					$pool_clauses []= " structure_cd LIKE 'RN%' " ;
			}
			/*
			if(in_array('all_com', $improvements_all)){
				$prop_clauses []= " prop_class LIKE '4%' " ;
			} 
			if(in_array('all_res_exclude', $improvements_all)){
				$prop_clauses []= " prop_class NOT LIKE '2%' " ;
			} 
			if(in_array('all_com_exclude', $improvements_all)){
				$prop_clauses []= " prop_class NOT LIKE '4%' " ;
			} 
			*/
			$sqlWHERE .= implode(" AND ", $pool_clauses) . ") ";
